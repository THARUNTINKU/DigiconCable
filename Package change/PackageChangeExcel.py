...
def getallvcno():
    workbook = xlrd.open_workbook("I:/Digicon/Digicon/PackageChange.xlsx")
    sheet = workbook.sheet_by_name("Pack1to4")
    filtered_vcno = []
    flag = 1
    for curr_row in range(2, sheet.nrows):
        row_data = []
        for curr_col in range(0, 5):
            # Read the data in the current cell
            data = sheet.cell_value(curr_row, curr_col)
